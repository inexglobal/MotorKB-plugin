#ifndef __MOTOR_KB_CPP__
#define __MOTOR_KB_CPP__

#include "motorkb.h"

MotorKB::MotorKB(int bus_ch, int dev_addr) {
	channel = bus_ch;
	address = dev_addr;
	polling_ms = 40; // Not not use
}

void MotorKB::init(void) {
...
int MotorKB::prop_count(void) {
	// not supported
	return 0;
}

bool MotorKB::prop_name(int index, char *name) {
	// not supported
	return false;
}

bool MotorKB::prop_unit(int index, char *unit) {
	// not supported
	return false;
}

bool MotorKB::prop_attr(int index, char *attr) {
	// not supported
	return false;
}

bool MotorKB::prop_read(int index, char *value) {
	// not supported
	return false;
}

bool MotorKB::prop_write(int index, char *value) {
	// not supported
	return false;
}
// --------------------------------------
...
// Start here
void MotorKB::process(Driver *drv) {
...
// Method
void MotorKB::i2c_setClock(uint32_t clock) {
	// Reset speed of I2C
	i2c_config_t conf;
...
bool MotorKB::send(uint8_t command) {
...
bool MotorKB::send(uint8_t command, uint8_t parameter) {
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_WRITE, true);
	i2c_master_write_byte(cmd, command, true);
...
bool MotorKB::send(uint8_t command, uint8_t parameter, int request_length) {
...
bool MotorKB::send(uint8_t command, int request_length) {
	i2c_cmd_handle_t cmd;
	esp_err_t ret;
...
uint8_t MotorKB::digital_read(uint8_t ch, bool pullup) {
...
bool MotorKB::digital_write(uint8_t ch, uint8_t value) {
	if (ch > 7) {
		return false;
	}
...
int MotorKB::analog_read(uint8_t ch) {
	if (ch > 7) {
		return 0;
	}
...
bool MotorKB::motor(uint8_t ch, uint8_t dir, uint8_t speed) {
	if (ch < 1 || ch > 4) {
		return false;
	}
	speed = fmax(speed, 0);
...
bool MotorKB::servo(uint8_t ch, uint8_t angle) {
	if (ch < 1 || ch > 6) {
		return false;
	}
...
bool MotorKB::servo2(uint8_t ch, uint8_t dir, uint8_t speed) {
	if (ch < 1 || ch > 6) {
		return false;
	}
...
bool MotorKB::uart_config(unsigned long baud) {
	uartBaud = baud;
	if (baud == 2400) {
		baudToBit = 0b00;
	} else if (baud == 9600) {
...
bool MotorKB::uart_write(char data) {
	char dataStr[] = { data, 0 };
	return uart_write(dataStr);
}

bool MotorKB::uart_write(bool data) {
	return uart_write((char)('0' + (data ? 1 : 0)));
}

bool MotorKB::uart_write(double data) {
...
bool MotorKB::uart_write(const char* data) {
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_WRITE, true);
...
bool MotorKB::uart_write_line(char data) {
	char dataStr[] = { data, 0 };
	return uart_write_line(dataStr);
}

bool MotorKB::uart_write_line(bool data) {
	return uart_write_line((char)('0' + (data ? 1 : 0)));
}

bool MotorKB::uart_write_line(double data) {
	char dataStrBuffer[20];
	memset(dataStrBuffer, 0, sizeof dataStrBuffer);
...
bool MotorKB::uart_write_line(const char* data) {
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_WRITE, true);
	i2c_master_write_byte(cmd, 0x04 | baudToBit, true);
...
uint16_t MotorKB::uart_available() {
	uint8_t newDataInMotorKB = 0;
	uint8_t readCount = 0;
	...
	newDataInMotorKB = read_data[0];
	ESP_LOGI("iKB-1", "Data new (iKB-1): %d", newDataInMotorKB);

	while(newDataInMotorKB > 0) {
		readCount = uart_read_from_motorkb(newDataInMotorKB);
		...
		newDataInMotorKB = newDataInMotorKB - readCount;
...
char MotorKB::uart_read() {
	xQueueReceive(uartReadQueue, &strBuffer[0], 10);
	
	return strBuffer[0];
}

char* MotorKB::uart_read(uint8_t count) {
...
int MotorKB::uart_read_from_motorkb(uint8_t count) {
...
char* MotorKB::uart_read_string(uint32_t timeout) {
...
char* MotorKB::uart_read_line(uint32_t timeout) {
	uart_read_until("\n", timeout);
	
	return strBuffer;
}

char* MotorKB::uart_read_until(char* until, uint32_t timeout) {
	uint32_t wait_max_time = timeout;
	uint32_t lastUpdate = esp_timer_get_time();
	uint32_t nowTime;
